...
class plgVmcustomDownloadableInstallerScript
{
	...
	public function uninstall($adapter)
	{
		...
		if ($virtuemart_custom_id)
			$this->db->setQuery("DELETE FROM {$this->dbprefix}virtuemart_product_customfields, {$this->dbprefix}virtuemart_customs WHERE virtuemart_custom_id = {$virtuemart_custom_id}")->query();
		return $this->reportIfDbError();
	}
